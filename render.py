...
class TerminalRenderer:
    ...
    def _redraw_input_status(self):
        ...
        为什么需要它：
          ...
          - 因此只把光标上移到状态区顶部，逐行清行并重写每个状态槽，最后光标回落到
            输入行原有输入内容之后——用户输入原样保留。

        步骤：
          1. 光标从输入行上移 status_lines 行到状态区首行。
          2. 对每个状态槽：回到行首 + 清行 + 写「{key}: {text}」+ 下移一行。
          3. 写完所有槽后，最后一次下移正好回到输入行。
        游标跟踪：每个槽写内容时 \r 回到行首、写完后 \n 下移一行，所以每写完一个槽
        游标精确下移 1 行；写入 N 个槽之后，做 N 次「内容 + 下移」等价于回到输入行。
        （旧实现写完带 \n 的槽后仍多下移一行并补一次空写，导致游标落错行、状态刷屏。）"""
        total = self._status_lines
        # 光标当前在输入行，上移 total 行回到状态区首行
        out = f"\r{_CSI_UP}{total}A"
        for key in self.slots:
            # 行首 + 清行 + 内容 + 换行(下移一行)；最后一个换行恰好把游标带回输入行
